...
MotionData IMU::MotionModel(double t)
{
	...
	float K = 3.1415926535897932384626 / 10.0;    // 20 * K = 2pi 　　由于我们采取的是时间是20s, 系数K控制yaw正好旋转一圈，运动一周
...
void IMU::testImu(std::string src, std::string dist)
{
	...
	//init last date
	//Eigen::Vector3d Pwb_last(init_twb_);              // position :    from  imu measurements
	//Eigen::Quaterniond Qwb_last(init_Rwb_);            // quaterniond:  from imu measurements
	//Eigen::Vector3d Vw_last(init_velocity_);          // velocity  :   from imu measurements
	double imu_time_last = imudata[0].timestamp;
	Eigen::Vector3d acc_w_last = Qwb * (imudata[0].imu_acc - acc_bias_) + gw;
	Eigen::Vector3d imu_gyro_last = imudata[0].imu_gyro;
	for (int i = 1; i < imudata.size(); ++i) {
		...
		/// 中值积分
		dt = imupose.timestamp - imu_time_last;
		Eigen::Vector3d imu_gyro_curr = imupose.imu_gyro;
		Eigen::Vector3d mean_gyro = (imu_gyro_last + imu_gyro_curr) * 0.5;
		...
		Qwb = Qwb * dq;
		Eigen::Vector3d acc_w_curr = Qwb * (imupose.imu_acc) + gw;
		Eigen::Vector3d acc_w_mean = (acc_w_last + acc_w_curr) * 0.5;

		Pwb = Pwb + Vw * dt + 0.5 * dt * dt * acc_w_mean;
		Vw = Vw + acc_w_mean * dt;
		
		imu_time_last = imupose.timestamp;
		acc_w_last = acc_w_curr;
		imu_gyro_last = imu_gyro_curr;
